...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function go(){
          ...
          window.location.href="${pageContext.request.contextPath}/task.do?flag=showTask1&userId=${loginuser.userId}&pageNow="+pageNow;
        };
...
<div class="container clearfix">
    ...
    <center>
    <h4>任务信息</h4>
    <form action="${pageContext.request.contextPath}/task.do?flag=showQTask1&userId=${loginuser.userId}" method="post">
    ...
    <td align="center"><a href="${pageContext.request.contextPath}/task.do?flag=goFenTask&id=${task.id}&userId=${loginuser.userId}" class="btn btn-primary btn2">分配任务</a>   
    </tr>
    </c:forEach>
    </table>
         <c:if test="${pageNow>1 }">
       <a href="${pageContext.request.contextPath}/task.do?flag=showTask1&userId=${loginuser.userId}&pageNow=${pageNow-1 }"><input type="button" class="btn btn-primary btn2" value="上一页" /></a>  
    </c:if>
    <c:forEach var="i" begin="1" end="${pageCount}" >
        <a href="${pageContext.request.contextPath}/task.do?flag=showTask1&userId=${loginuser.userId}&pageNow=${i}">[${i }]</a>
    </c:forEach>
    <c:if test="${pageNow<pageCount }">
      <a href="${pageContext.request.contextPath}/task.do?flag=showTask1&userId=${loginuser.userId}&pageNow=${pageNow+1 }"><input type="button" class="btn btn-primary btn2" value="下一页" /></a>
    </c:if>
