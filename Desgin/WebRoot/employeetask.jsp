...
<div class="container clearfix">
    ...
    <center>
    <h4>个人任务</h4>
    <form action="${pageContext.request.contextPath}/task.do?flag=showQEmpTask&userId=${loginuser.userId}" method="post">
    ...
    <td width=125px align="center">分配日期</td>
    ...
    <td align="center">${task.fenTime}</td>
    <td align="center">${task.grade}</td><td align="center">${task.type}</td>
    <td align="center"><a href="${pageContext.request.contextPath}/task.do?flag=goEmpTaskType&id=${task.id}" class="btn btn-primary btn2">开始任务</a>   
    </tr>
    </c:forEach>
    </table>
         <c:if test="${pageNow>1 }">
       <a href="${pageContext.request.contextPath}/task.do?flag=showEmpTask&userId=${loginuser.userId}&pageNow=${pageNow-1 }"><input type="button" class="btn btn-primary btn2" value="上一页" /></a>  
    </c:if>
    <c:forEach var="i" begin="1" end="${pageCount}" >
        <a href="${pageContext.request.contextPath}/task.do?flag=showEmpTask&userId=${loginuser.userId}&pageNow=${i}">[${i }]</a>
    </c:forEach>
    <c:if test="${pageNow<pageCount }">
      <a href="${pageContext.request.contextPath}/task.do?flag=showEmpTask&userId=${loginuser.userId}&pageNow=${pageNow+1 }"><input type="button" class="btn btn-primary btn2" value="下一页" /></a>
    </c:if>
