...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function go(){
          ...
          window.location.href="${pageContext.request.contextPath}/task.do?flag=showFenTask&userId=${loginuser.userId}&pageNow="+pageNow;
        };
...
<div class="container clearfix">
    ...
    <center>
    <h4>任务信息检视</h4>
    <form action="${pageContext.request.contextPath}/task.do?flag=showQFenTask" method="post">
    ...
    <td width=150px align="center">任务名称</td><td width=125px align="center">创建人编号</td>
    <td width=125px align="center">创建人名称</td><td width=125px align="center">任务创建日期</td>
    <td width=125px align="center">任务完成日期</td><td width=100px align="center">任务的级别</td>
    <td width=80px align="center">职工编号</td><td width=80px align="center">职工名称</td>
    <td width=150px align="center">分配日期</td><td width=80px align="center">任务状态</td>
    <td width=110px align="center">操作</td>
    </tr>
    <c:forEach items="${tasklist}" var="task">
    <tr>
    <td align="center">${task.id}</td><td align="center">${task.taskId}</td>
    <td align="center">${task.taskName}</td><td align="center">${task.founderId}</td>
    <td align="center">${task.founder}</td><td align="center">${task.startTime}</td>
    <td align="center">${task.endTme}</td><td align="center">${task.grade}</td>
    <td align="center">${task.emploeeId}</td><td align="center">${task.employeeName}</td>
    <td align="center">${task.fenTime}</td><td align="center">${task.type}</td>
    <td align="center"><a href="${pageContext.request.contextPath}/task.do?flag=goFenTask&id=${task.id}&userId=${loginuser.userId}" class="btn btn-primary btn2">修改分配</a>   
    </tr>
    </c:forEach>
    </table>
         <c:if test="${pageNow>1 }">
       <a href="${pageContext.request.contextPath}/task.do?flag=showFenTask&userId=${loginuser.userId}&pageNow=${pageNow-1 }"><input type="button" class="btn btn-primary btn2" value="上一页" /></a>  
    </c:if>
    <c:forEach var="i" begin="1" end="${pageCount}" >
        <a href="${pageContext.request.contextPath}/task.do?flag=showFenTask&userId=${loginuser.userId}&pageNow=${i}">[${i }]</a>
    </c:forEach>
    <c:if test="${pageNow<pageCount }">
      <a href="${pageContext.request.contextPath}/task.do?flag=showFenTask&userId=${loginuser.userId}&pageNow=${pageNow+1 }"><input type="button" class="btn btn-primary btn2" value="下一页" /></a>
    </c:if>
